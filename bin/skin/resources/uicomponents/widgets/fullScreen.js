...
}XWiki.widgets.FullScreen=Class.create({margin:0,buttonSize:16,initialize:function(){this.buttons=$(document.body).down(".bottombuttons");
...
$$("textarea",".maximizable").each(function(B){this.addBehavior(B)
}.bind(this));
...
},addElementButton:function(A){Element.insert(A,{before:this.createOpenLink(A)})
...
},createOpenButton:function(B){var A=new Element("img",{"class":"fullScreenEditButton",title:"Maximize",alt:"Maximize",src:"/resources/icons/silk/arrow_out.gif"});
...
var A=new Element("a",{"class":"fullScreenEditLink",title:"Maximize"});
A.update("Maximize &raquo;");
A.observe("click",this.makeFullScreen.bind(this,B));
C.update(A);
B._x_fullScreenActivator=A;
A._x_maximizedElement=B;
return C
},createCloseButtons:function(){this.closeButton=new Element("img",{"class":"fullScreenCloseButton",title:"Exit full screen",alt:"Exit full screen",src:"/resources/icons/silk/arrow_in.gif"});
...
this.actionCloseButton=new Element("input",{type:"button","class":"button",value:"Exit full screen"});
...
document.observe("xwiki:dom:loaded",function(){new XWiki.widgets.FullScreen()
});
